...
function STL(file_path){
        ...
        this.identical = Array.from(new Array(95), (x,i) => i+32)
        this.identical = this.identical.concat([0x00, 0xa0, 0xa1, 0xa2, 0xa3, 0xa5, 0xa7, 0xab, 0xb0, 0xb1, 0xb2, 0xb3, 0xb5, 0xb6, 0xb7, 0xbb, 0xbc, 0xbd, 0xbe, 0xbf])
...
STL.prototype.decode_text = function(input){
    let output = ""
    for (let charInd in input){
        let char = input[charInd];
        if(this.identical.includes(char)){
            output +=String.fromCharCode(char);
        } else if(char in this.direct_mapping){
            output += this.direct_mapping[char];
        }
    }
    return output;
}
...
STL.prototype.decode = function(){
    // input should be STL File
    // output should be subtitles objects
    let data = null;
    try {  
    ...
    for(let n = 0; n<k; n++){
        ...
        let tti = {
            ...
            startTime:            time_2_tcr(tti_block.slice(5,9)),
            endTime:              time_2_tcr(tti_block.slice(9,13)),
            ...
            text:                 this.decode_text(tti_block.slice(16,128)),
        }
        stl_output.push(tti);
    }
    this.ttiData = stl_output;

    return({"gsiData":gsi, "ttiData":stl_output})
}

STL.prototype.encode = function(stlData){
    // input should be subtitles objects
    // output should be STL Buffer

    // encode GSI Block
    let gsiData = null;
    if("gsiData" in stlData){
        let gsi_buf = Buffer.alloc(1024);

        // code page number
        let gsiData = stlData['gsiData'];
        console.log(gsi_buf.length);
    }else{
        console.log("GSI Data not found")
    }
